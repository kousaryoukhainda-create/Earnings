...
public final class zaaa implements zaca {
    ...
    /* JADX WARN: Type inference failed for: r1v5, types: [java.util.Map, Z10] */
    private zaaa(Context context, zabe zabeVar, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map map, Map map2, ClientSettings clientSettings, Api.AbstractClientBuilder abstractClientBuilder, Api.Client client, ArrayList arrayList, ArrayList arrayList2, Map map3, Map map4) {
        ...
        for (Api.AnyClientKey anyClientKey : map2.keySet()) {
            z10.put(anyClientKey, this.zad);
        }
        for (Api.AnyClientKey anyClientKey2 : map.keySet()) {
            z10.put(anyClientKey2, this.zae);
        }
        this.zaf = Collections.unmodifiableMap(z10);
    }
    ...
    /* JADX WARN: Type inference failed for: r13v0, types: [java.util.Map, Z10] */
    /* JADX WARN: Type inference failed for: r14v0, types: [java.util.Map, Z10] */
    /* JADX WARN: Type inference failed for: r6v0, types: [java.util.Map, Z10] */
    /* JADX WARN: Type inference failed for: r7v0, types: [java.util.Map, Z10] */
    public static zaaa zag(Context context, zabe zabeVar, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map map, ClientSettings clientSettings, Map map2, Api.AbstractClientBuilder abstractClientBuilder, ArrayList arrayList) {
        ...
        for (Map.Entry entry : map.entrySet()) {
            ...
            if (client2.requiresSignIn()) {
                z10.put((Api.AnyClientKey) entry.getKey(), client2);
            } else {
                z102.put((Api.AnyClientKey) entry.getKey(), client2);
            }
        }
        Preconditions.checkState(!z10.isEmpty(), "CompositeGoogleApiClient should not be used without any APIs that require sign-in.");
        ?? z103 = new Z10();
        ?? z104 = new Z10();
        for (Api api : map2.keySet()) {
            Api.AnyClientKey zab = api.zab();
            if (z10.containsKey(zab)) {
                z103.put(api, (Boolean) map2.get(api));
            } else if (z102.containsKey(zab)) {
                z104.put(api, (Boolean) map2.get(api));
            } else {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (z103.containsKey(zatVar.zaa)) {
                arrayList2.add(zatVar);
            } else if (z104.containsKey(zatVar.zaa)) {
                arrayList3.add(zatVar);
            } else {
                throw new IllegalStateException("Each ClientCallbacks must have a corresponding API in the isOptionalMap");
            }
        }
        return new zaaa(context, zabeVar, lock, looper, googleApiAvailabilityLight, z10, z102, clientSettings, abstractClientBuilder, client, arrayList2, arrayList3, z103, z104);
    }
